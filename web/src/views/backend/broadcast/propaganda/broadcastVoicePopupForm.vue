  ...
  <el-dialog
    ...
    <el-row>
      <el-table :data="broadcastRecordingData">
        <el-table-column property="voice_file_name" label="文件名称"/>
        <el-table-column property="voice_file_url" label="文件地址" width="250px"/>
...
const emits = defineEmits(['broadcastFinish', 'openDialogTableVisible']);
...
function playRecordingBroadcast(broadcastData) {
    ...
    console.log("[" + Date.now() + "-音频广播日志]" + "：开始播放音频广播!");
    // 更新音频文件数据
...
function calculateDuration(broadcastRecordingData) {
    ...
    broadcastRecordingData.value.forEach((item, index) => {
        ...
        console.log("计算播放时间timeString", timeString)
        // 将时间字符串转换为毫秒
