...
import { indexMusicDatas } from '/@/api/common'
        ...
        indexBroadcastTasks()
            .then((response) => {
                ...
                response.data.list.forEach((item, index) => {
                    // 如果时间相等
                    if (Math.trunc(Date.parse(item.start_datetime)/1000/60 - Date.now()/1000/60) === 0) {
                        console.log('定时广播开始，开始定时广播的id为：' + item.task_id);
                        // 将回传需要广播的音频数据存储到broadcastRecordingData里面
                        indexMusicDatas().then((response) => {
                            broadcastRecordingData.value = response.data.list;
                            console.log("音乐 :" + response)
                            console.log("broadcastRecordingData.value : " + broadcastRecordingData.value)
